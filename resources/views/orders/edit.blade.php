...
<script>
    ...
    function buildHTMLProductsList(snapshotsProducts) {
        ...
        if (currencyAtRight) {
            ...
            html = html + '<td  id="greenColor">' + price_val + '</td><td> × ' + val.quantity + '</td><td id="greenColor"> + ' + extras_price_val + '</td><td  id="greenColor">  ' + totalProductPrice_val + '</td>';
    ...
    function buildHTMLProductstotal(snapshotsProducts) {
        ...
        if (intRegex.test(discount) || floatRegex.test(discount)) {
            ...
            html = html + '<tr><td class="label">{{trans("lang.discount")}}' + couponCode_html + '</td><td class="discount text-danger">(' + discount_val + ')</td></tr>';
        }
        ...
        if(specialDiscount != undefined){
            ...
            html = html + '<tr><td class="label">{{trans("lang.special_offer")}} {{trans("lang.discount")}}' + special_html + '</td><td class="special_discount text-danger">(' + special_discount_val + ')</td></tr>';
        }
    ...
    try{
        ...
        if(!isNaN(tax) && tax!=0){
        if(currencyAtRight){
            html=html+'<tr><td class="label">{{trans("lang.tax")}}</td><td class="tax_amount" id="greenColor">+'+tax.toFixed(decimal_degits)+''+currentCurrency+'('+taxlabel+' '+snapshotsProducts.taxSetting.tax+' '+taxlabeltype+')</td></tr>';
        }else{
            html=html+'<tr><td class="label">{{trans("lang.tax")}}</td><td class="tax_amount" id="greenColor">+'+currentCurrency+tax.toFixed(decimal_degits)+'('+taxlabel+' '+snapshotsProducts.taxSetting.tax +' '+taxlabeltype+')</td></tr>';
        }
        ...
        if (intRegex.test(deliveryCharge) || floatRegex.test(deliveryCharge)) {
            ...
            if (takeAway == '' || takeAway == false) {
                ...
                html = html + '<tr><td class="label">{{trans("lang.deliveryCharge")}}</td><td class="deliveryCharge " id="greenColor">+' + deliveryCharge_val + '</td></tr>';
            }
        }
        ...
        if (intRegex.test(tip_amount) || floatRegex.test(tip_amount)) {
            ...
            if (takeAway == '' || takeAway == false) {
                ...
                html = html + '<tr><td class="label">{{trans("lang.tip_amount")}}</td><td class="tip_amount_val " id="greenColor">+' + tip_amount_val + '</td></tr>';
            }
        }
        ...
        html = html + '<tr><td class="label">{{trans("lang.total_amount")}}</td><td class="total_price_val " id="greenColor">' + total_price_val + '</td></tr>';
