...
- $ 0
                            ...
                            <dd class="col-6">
                                <label class="total_tax_amount">+ $ 0</label>
                            </dd>
    ...
    function buildHTMLProductstotal(snapshotsProducts) {
            ...
            if (snapshotsProducts.hasOwnProperty('taxSetting')) {
                if (snapshotsProducts.taxSetting.type && snapshotsProducts.taxSetting.tax) {
                    if (snapshotsProducts.taxSetting.type == "percent") {
                        tax = parseFloat((snapshotsProducts.taxSetting.tax * total_price) / 100).toFixed(
                        decimal_degits);
                        taxlabeltype = "%";
                    } else {
        ...
        if (!isNaN(tax) && tax != 0) {
            if (currencyAtRight) {
                html = html + '<tr><td class="label">{{trans("lang.tax")}}</td><td class="deliveryCharge">+' + tax +
                    '' + currentCurrency + '(' + taxlabel + ' ' + snapshotsProducts.taxSetting.tax + ' ' +
                    taxlabeltype + ')</td></tr>';
                $('.total_tax_amount').text("+ " + tax + " " + currentCurrency + ' (' + taxlabel + ' ' +
                    snapshotsProducts.taxSetting.tax + ' ' + taxlabeltype + ')');
            } else {
                html = html + '<tr><td class="label">{{trans("lang.tax")}}</td><td class="deliveryCharge">+' +
                    currentCurrency + tax + '(' + taxlabel + ' ' + snapshotsProducts.taxSetting.tax + ' ' +
                    taxlabeltype + ')</td></tr>';
                $('.total_tax_amount').text("+ " + currentCurrency + "" + tax + '(' + taxlabel + ' ' + snapshotsProducts
                    .taxSetting.tax + ' ' + taxlabeltype + ')');
            }
